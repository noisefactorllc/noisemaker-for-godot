...
layout(set = 0, binding = 1) uniform sampler2D xyzTex;      // [x, y, heading, alive] in normalized [0,1]
layout(set = 0, binding = 2) uniform sampler2D velTex;      // [0, 0, age, seed]
layout(set = 0, binding = 3) uniform sampler2D rgbaTex;     // [r, g, b, a]
layout(set = 0, binding = 4) uniform sampler2D trailTex;    // Trail texture for sensor feedback
layout(set = 0, binding = 5) uniform sampler2D inputTex;    // Input texture for field attraction
