...
layout(set = 0, binding = 1) uniform sampler2D xyzTex;      // [x, y, 0, alive] - normalized coords
layout(set = 0, binding = 2) uniform sampler2D velTex;      // [vx, vy, age, seed]
layout(set = 0, binding = 3) uniform sampler2D rgbaTex;     // [r, g, b, a]
layout(set = 0, binding = 4) uniform sampler2D dataTex;     // [typeId, mass, 0, 0] - internal
layout(set = 0, binding = 5) uniform sampler2D forceMatrix; // [strength, prefDist, curveShape, 1]
layout(set = 0, binding = 6) uniform sampler2D inputTex;    // Source texture for color sampling
...
void main() {
	...
	ivec2 stateTextureSize = textureSize(xyzTex, 0);
	...
	vec4 particleData = texelFetch(dataTex, coord, 0);
	...
	float typeId = particleData.x;
	float mass = particleData.y;

	uint particleId = uint(coord.x + coord.y * stateTextureSize.x);
	...
	int totalParticles = stateTextureSize.x * stateTextureSize.y;
    ...
    for (int dy = -1; dy <= 1; dy++) {
        for (int dx = -1; dx <= 1; dx++) {
            ...
            for (int s = 0; s < 12; s++) {
				...
				int sx = sampleIdx % stateTextureSize.x;
				int sy = sampleIdx / stateTextureSize.x;
	...
	if (useTypeColor != 0.0) {
		outRGBA = vec4(typeColor(int(typeId), int(typeCount)), 1.0);
    } else {
