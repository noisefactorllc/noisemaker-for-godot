...
layout(set = 0, binding = 1) uniform sampler2D xyzTex;    // [x, y, z, alive]
layout(set = 0, binding = 2) uniform sampler2D velTex;    // [seed, justStuck, 0, agentRand]
layout(set = 0, binding = 3) uniform sampler2D rgbaTex;   // [r, g, b, a]

// DLA internal textures
layout(set = 0, binding = 4) uniform sampler2D gridTex;   // Anchor grid (internal, not rendered)
layout(set = 0, binding = 5) uniform sampler2D inputTex;  // For input-weighted movement
