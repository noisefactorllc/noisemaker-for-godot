...
vec3 computeFractal(vec3 p, vec3 juliaC) {
    if (int(noiseType) == 0) {
        return mandelbulb(p, power, int(iterations), bailout);
    } else if (int(noiseType) == 1) {
        float scale = clamp(power * 0.25, -3.0, 3.0);
        return mandelcube(p, scale, int(iterations), bailout);
    } else if (int(noiseType) == 2) {
        return juliaBulb(p, juliaC, power, int(iterations), bailout);
    } else {
        float scale = clamp(power * 0.25, -3.0, 3.0);
        return juliaCube(p, juliaC, scale, int(iterations), bailout);
    }
}
...
void main() {
    ...
    // Output volume data based on int(colorMode)
    // int(colorMode) 0 = mono (grayscale), 1 = rgb (distance, trap, iteration)
    if (int(colorMode) == 0) {
