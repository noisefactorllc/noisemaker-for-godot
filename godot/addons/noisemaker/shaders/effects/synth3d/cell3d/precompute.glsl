...
// Atlas layout: int(volumeSize) x (int(volumeSize) * int(volumeSize))
// Pixel (x, y) maps to 3D coordinate (x, y % int(volumeSize), y / int(volumeSize))
...
void main() {
    ...
    // Normalize distance based on int(metric)
    float normalizer;
    if (int(metric) == 0) {
    ...
    // Pack output based on int(colorMode)
    // int(colorMode) 0 = mono (grayscale), 1 = rgb (cell colors)
    if (int(colorMode) == 0) {
