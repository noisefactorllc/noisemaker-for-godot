#version 450
layout(set = 0, binding = 1) uniform sampler2D stateTex;
...
// Sample int(seed) texture at voxel coordinate (for inputTex3d seeding)
...
void main() {
    ...
    if (bufferIsEmpty || (resetState > 0.5)) {
        ...
        if (hasSeedInput) {
            // Use int(seed) texture luminance to determine initial alive state
    ...
    if (alive > 0.5) {
        ...
        if (shouldSurvive(neighbors, int(ruleIndex))) {
            newAlive = 1.0;
        ...
        if (shouldBeBorn(neighbors, int(ruleIndex))) {
            newAlive = 1.0;
            newAge = 0.0;
        } else {
            newAlive = 0.0;
