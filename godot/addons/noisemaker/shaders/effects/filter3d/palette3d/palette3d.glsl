#version 450
layout(set = 0, binding = 1) uniform sampler2D inputTex3d;
/*
...
vec3 cosinePalette(float t, vec3 amp, vec3 freq, vec3 bias, vec3 phase) {
...
void main() {
    ...
    if (int(paletteIndex) <= 0 || int(paletteIndex) > PALETTE_COUNT) {
    ...
    // Apply palette modifiers: repeat, offset, and int(rotation) (animation)
    float t = lum * repeat + offset * 0.01;
    if (int(rotation) == -1) {
        t += time;
    } else if (int(rotation) == 1) {
        t -= time;
    }

    // Get palette entry (array is 0-indexed, palette indices are 1-indexed)
    PaletteEntry entry = PALETTES[int(paletteIndex) - 1];

    // Extract mode from amp.w
    int mode = int(entry.amp.w);

    // Apply cosine palette
    vec3 paletteColor = cosinePalette(t, entry.amp.xyz, entry.freq.xyz, entry.offsetValue.xyz, entry.phase.xyz);
