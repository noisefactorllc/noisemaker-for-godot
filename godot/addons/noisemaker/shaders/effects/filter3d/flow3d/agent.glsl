#version 450
layout(set = 0, binding = 1) uniform sampler2D stateTex1;
layout(set = 0, binding = 2) uniform sampler2D stateTex2;
layout(set = 0, binding = 3) uniform sampler2D stateTex3;
layout(set = 0, binding = 4) uniform sampler2D mixerTex;
/*
...
float computeRotationBias(float baseHeading, float baseRotRand, float t, int agentIndex, int totalAgents) {
#if BEHAVIOR <= 0
    return 0.0;
#elif BEHAVIOR == 1
...
#elif BEHAVIOR == 10
    // Meandering
    return normalized_sine((t - baseRotRand) * TAU);
#else
    return baseRotRand * TAU;
#endif
}
