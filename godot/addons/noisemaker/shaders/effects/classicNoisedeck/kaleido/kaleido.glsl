...
float constantNoise(vec2 st, float freq) {
...
float quadratic3x3Value(vec2 st, float freq) {
    ...
    // Row -1 (y-1)
    float v00 = constantNoise(st + vec2(-nd, -nd), freq);
    float v10 = constantNoise(st + vec2(0.0, -nd), freq);
    float v20 = constantNoise(st + vec2(nd, -nd), freq);

    // Row 0 (y)
    float v01 = constantNoise(st + vec2(-nd, 0.0), freq);
    float v11 = constantNoise(st, freq);
    float v21 = constantNoise(st + vec2(nd, 0.0), freq);

    // Row 1 (y+1)
    float v02 = constantNoise(st + vec2(-nd, nd), freq);
    float v12 = constantNoise(st + vec2(0.0, nd), freq);
    float v22 = constantNoise(st + vec2(nd, nd), freq);
...
float bicubicValue(vec2 st, float freq) {
    ...
    float x0y0 = constantNoise(vec2(u0, v0), freq);
    float x0y1 = constantNoise(vec2(u0, v1), freq);
    float x0y2 = constantNoise(vec2(u0, v2), freq);
    float x0y3 = constantNoise(vec2(u0, v3), freq);

    float x1y0 = constantNoise(vec2(u1, v0), freq);
    float x1y1 = constantNoise(st, freq);
    float x1y2 = constantNoise(vec2(u1, v2), freq);
    float x1y3 = constantNoise(vec2(u1, v3), freq);

    float x2y0 = constantNoise(vec2(u2, v0), freq);
    float x2y1 = constantNoise(vec2(u2, v1), freq);
    float x2y2 = constantNoise(vec2(u2, v2), freq);
    float x2y3 = constantNoise(vec2(u2, v3), freq);

    float x3y0 = constantNoise(vec2(u3, v0), freq);
    float x3y1 = constantNoise(vec2(u3, v1), freq);
    float x3y2 = constantNoise(vec2(u3, v2), freq);
    float x3y3 = constantNoise(vec2(u3, v3), freq);
...
float value(vec2 st, float freq, int interp) {
    ...
    float x1y2 = constantNoise(vec2(st.x, st.y + ndY), freq);
    float x2y1 = constantNoise(vec2(st.x + ndX, st.y), freq);
    float x2y2 = constantNoise(vec2(st.x + ndX, st.y + ndY), freq);
