#version 450
/**
 * Remap - GLSL fragment shader (reference 0ed489ec — full polygon-zone router rewrite)
 *
...
layout(set = 0, binding = 0, std140) uniform RemapUniforms { vec4 data[275]; };

// Godot never tiles a render (no tiled-export path), so tileOffset is always zero and
// fullResolution is the same value the packed UBO carries for tile-local sampling.
#define tileOffset vec2(0.0)
#define fullResolution data[RESOLUTION_SLOT].xy
// Per-zone source surfaces. Wired in DSL via `zoneN_tex: read(oN)`.
layout(set = 0, binding = 1) uniform sampler2D zone0_tex;
layout(set = 0, binding = 2) uniform sampler2D zone1_tex;
layout(set = 0, binding = 3) uniform sampler2D zone2_tex;
layout(set = 0, binding = 4) uniform sampler2D zone3_tex;
layout(set = 0, binding = 5) uniform sampler2D zone4_tex;
layout(set = 0, binding = 6) uniform sampler2D zone5_tex;
layout(set = 0, binding = 7) uniform sampler2D zone6_tex;
layout(set = 0, binding = 8) uniform sampler2D zone7_tex;
