...
layout(set = 0, binding = 1) uniform sampler2D preparedTex;  // Original rotated/prepared image
layout(set = 0, binding = 2) uniform sampler2D rankTex;      // R = rank (approx), G = luminance, B = original x
layout(set = 0, binding = 3) uniform sampler2D brightestTex; // R = brightest x per row
