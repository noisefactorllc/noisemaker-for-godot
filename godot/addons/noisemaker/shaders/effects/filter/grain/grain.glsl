...
// GLSL golden's own source, so it is the port source here). Film grain overlay:
// blends the source image with animated bicubic value noise. Reverted to the
// reference's current (simple) two-param surface — alpha/pause only; a prior
// GRAIN_TYPE/intensity/contrast/mono extension does not exist upstream anymore
...
// names directly. `pause` is injected as a raw float (boolean param), tested as
// int(pause) > 0 (this port's established idiom; matches the reference GLSL's
// own `pause > 0.5` comparison). Input texture bound at set 0, binding 1.
