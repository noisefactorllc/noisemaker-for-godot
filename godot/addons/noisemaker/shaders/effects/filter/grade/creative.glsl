...
vec3 applyVibrance(vec3 rgb, float vibranceAmount) {
	if (abs(vibranceAmount) < 0.001) return rgb;
	...
	float vibranceGain = 1.0 + vibranceAmount * (1.0 - sat);
...
vec3 applySplitTone(vec3 rgb, vec3 shadowTintValue, vec3 highlightTintValue, float balance) {
    // Tints are specified as 0.5 = neutral, deviation = color shift
	vec3 shadowShift = (shadowTintValue - 0.5) * 2.0;
	vec3 highlightShift = (highlightTintValue - 0.5) * 2.0;
