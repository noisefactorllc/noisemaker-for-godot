...
float computeVignette(vec2 uv, vec2 aspectScale, float midpoint, float roundness, float feather) {
    ...
    if (roundness > 0.0) {
        // Blend toward circle
		scale = mix(aspectScale, vec2(1.0), roundness);
    } else {
        // Enhance aspect ratio difference
		scale = mix(aspectScale, aspectScale * vec2(1.0 + abs(roundness), 1.0 - abs(roundness) * 0.5), -roundness);
    }
...
void main() {
	...
	if (fullRes.x > fullRes.y) {
		aspectScale = vec2(fullRes.x / fullRes.y, 1.0);
	} else {
		aspectScale = vec2(1.0, fullRes.y / fullRes.x);
	}

    // Compute vignette mask using global UV so center is full-image center
	float vignetteMask = computeVignette(globalUV, aspectScale, vignetteMidpoint,
