...
vec3 applyWhiteBalance(vec3 rgb, float temp, float tintAmount) {
    ...
    vec3 shift = vec3(
		...
		1.0 - tintAmount * 0.5,     // Green channel
...
vec3 applyTonalRanges(vec3 rgb, float highlightsAmount, float shadowsAmount, float whitesAmount, float blacksAmount) {
	...
	lumaAdjust += highlightsAmount * hWeight * 0.5;  // Scale factor for usable range
	lumaAdjust += shadowsAmount * sWeight * 0.5;
	lumaAdjust += whitesAmount * wWeight * 0.3;
	lumaAdjust += blacksAmount * bWeight * 0.3;
...
// S-curve contrast
vec3 applyContrast(vec3 rgb, float contrastAmount) {
	if (abs(contrastAmount) < 0.001) return rgb;
    ...
    float pivot = 0.5;
	float factor = 1.0 + contrastAmount;
