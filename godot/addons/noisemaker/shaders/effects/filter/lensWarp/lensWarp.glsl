...
void main() {
	...
	float frameAspect = fullRes.x / fullRes.y;
	...
	vec2 scaled = vec2(delta.x * frameAspect, delta.y);
	float maxRadius = length(vec2(frameAspect * 0.5, 0.5));
    float mask = pow(clamp(length(scaled) / maxRadius, 0.0, 1.0), 5.0);

    // Two independent Perlin noise fields for X and Y displacement
	vec2 noiseCoord = uv * vec2(frameAspect, 1.0);
