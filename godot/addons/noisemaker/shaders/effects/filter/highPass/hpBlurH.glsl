#version 450
// filter/highPass, program "hpBlurH" — ported from wgsl/hpBlurH.wgsl (same shape as
// filter/unsharpMask's usmBlurH, plus a stride widening for radius>32 so the bounded
// 32-tap loop still reaches the full requested radius). No-layout effect: the backend
// synthesizes the Params UBO and injects `#define radius data[..]`. Input at set 0,
// binding 1.
