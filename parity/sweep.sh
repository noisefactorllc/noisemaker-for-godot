...
tol_for() {
	case "$1" in
		...
		chrome|chromeLiquid) echo "16.001 0.999" ;; # sine tone curve (v=0.5+0.5*sin(h2*cycles*TAU+...)) plus pow(v,8) rim-specular boost amplify a sub-LSB cross-GPU height-field difference into a band-shift for 1-2 px out of 65536 (<=0.003%); SSIM 0.9999+, same isolated-outlier class as bulge/degauss/reliefPlaster
		...
		strokesSprayed) echo "3.001 0.999" ;; # sprayed mode's per-tap hash22 jitter (up to +-3px) shifts many of smear()'s up to 24x2 taps to sub-pixel positions; a sub-ULP cross-GPU bilinear-filter weight difference at a handful of near-boundary jittered taps accumulates into a 1-LSB-over-tolerance drift for 2 px out of 65536 (0.003%), SSIM 0.99994 -- same NEAREST/bilinear boundary-tie hazard class as uvRemap/refract, not a rotation-handedness bug (angled/dark/sumiE, which exercise the SAME rotate2D at fixed 45/135 degrees with no jitter, all pass bit-exact-class: max-abs-diff 1-2)
		strokesSmudge) echo "3.001 0.999" ;; # smudge mode's edge-following angle (degrees(atan(grad.y,grad.x))+90, same formula as filter/hatch's coloredPencil) is unstable right at the Sobel lumGradient's near-zero singularity (the gradMag<=1e-5 fixed-45-degree fallback boundary); a sub-ULP cross-GPU difference there can shift the smear direction enough to accumulate slightly different source texels for 8 px out of 65536 (0.012%), SSIM 0.99996 -- same discrete-angle-instability hazard class as the pre-existing hatchPencil entry above, not a rotation-handedness bug (angled/dark/sumiE, which exercise the SAME rotate2D at fixed 45/135 degrees, all pass bit-exact-class: max-abs-diff 1-2)
