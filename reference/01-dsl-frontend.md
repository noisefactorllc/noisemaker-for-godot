...
4. **Output/Source ref**: char is `o` or `s` AND `src[i+1]` is a digit. Greedily consume following digits. `o`→`OUTPUT_REF`, `s`→`SOURCE_REF`. Lexeme e.g. `o0`, `s3`. Enforce that non-member `OUTPUT_REF` references (not preceded by `DOT`) strictly match `o0`..`o7`; out-of-range throws `SyntaxError`.
